...
- **Wolt 找店：** 按支持的餐厅类别浏览候选，读取实际店名、评分和地址，不进入下单或支付。
- **QQ 消息：** 根据用户指定的联系人和正文编辑草稿或单次发送；当前真机演示联系人安全锁可通过环境变量配置。
- **Wolt → QQ：** 先验证餐厅信息，再将结构化结果组合为消息草稿或发送内容。
- **网易云音乐：** 解析准确歌曲名，通过官方深链播放，并使用 Android MediaSession 验证曲目和播放状态。
- **YouTube：** 通过免输入法搜索深链展示指定主题的视频结果。
- **系统闹钟：** 设置用户指定的有效时间，并读取 Android 系统状态复核结果。
- **零干扰保护：** 所有任务运行在 Shadow Display；持续监控 Display 0、焦点和输入法，发现越界立即停止。
...
当前产品回归范围覆盖 QQ、Wolt、网易云音乐、YouTube 和系统时钟，并包含 Wolt → QQ 复合任务。餐厅地址直接从 Wolt 店铺详情页读取，不依赖地图；地图能力不作为本次交付承诺。新 App 仍可尝试通用 GUI 能力，但不计入稳定演示范围。
...
QQ 当前演示环境通过 `QQ_TEST_RECIPIENT` 将真实发送锁定为“红枣桂花熊”，仅用于防止真机反复测试时误发给其他联系人，并非产品能力限制。底层会从每次用户指令中动态解析联系人并通过 Accessibility 精确匹配；移除该环境变量后即可向用户明确指定的其他联系人发送。收件人缺失、与演示安全锁不符或最终确认页校验失败时，任务都会在发送前停止。Wolt→QQ 使用两阶段编排：先验证店名、评分、地址，再用该结构化结果构建原文消息，不会把 OCR 未验证文本直接外发。真实发送仍必须在任务中提供完整正文并通过敏感动作门禁。
