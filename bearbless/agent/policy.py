...
_SENSITIVE = (
    "支付", "付款", "购买", "转账", "发送", "发给", "发送给", "发消息", "发信息", "发送信息", "删除", "下单", "授权",
)
...
def infer_task_mode(goal: str) -> TaskMode:
    ...
    effective = re.sub(
        r"(?:不要|无需|禁止|不许|不)(?:支付|付款|购买|转账|发送|删除|下单|授权|播放|暂停|修改|输入|保存|创建|添加|切换|开启|关闭)",
        "",
        goal,
    )
    # “告诉我” is a query, while “告诉<联系人>…” delegates an outbound
    # communication.  Treat the latter as sensitive even when a compound task
    # begins with query words such as “找一家店”.
    tells_other_person = re.search(r"告诉(?!我(?:们)?)([^，,：:\s]{1,40})", effective) is not None
    if any(word in effective for word in _SENSITIVE) or tells_other_person:
